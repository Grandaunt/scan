...
public class DatePickerView extends View {
    ...
    public void setSelected(String mSelectItem) {
        for (int i = 0; i < mDataList.size(); i++) {
            if (mDataList.get(i).trim().equals(mSelectItem.trim())) {
                SystemPrintln.out(mSelectItem+"  "+mDataList.get(i)+"  "+i);
                setSelected(i);
                break;
            }
        }
    ...
    private void init() {
        ...
        mPaint.setColor(ContextCompat.getColor(context, R.color.DatePickerViewText1));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        nPaint.setColor(ContextCompat.getColor(context, R.color.DatePickerViewText2));


        this.setOnSelectListener(new onSelectListener() {
            @Override
    ...
    private void drawData(Canvas canvas) {
        ...
        if (mDataList.size() > 0) {
            ...
            SystemPrintln.out("----mDataList.get(mCurrentSelected)" + mDataList.get(mCurrentSelected));
            // 绘制上方data
