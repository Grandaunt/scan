...
public class DialogDatePicker {
    ...
    private TextView tv_cancle, tv_select, hour_text, minute_text, year_text, month_text, day_text, tv_title;
    //只显示  时 分
    private boolean ShowTimeOnly = false;

    public DialogDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
    ...
    private void loadComponent() {
        for (int i = 0; i < year.size(); i++)
            year.set(i, year.get(i) + "年");
        for (int i = 0; i < month.size(); i++)
            month.set(i, month.get(i) + "月");
        for (int i = 0; i < day.size(); i++)
            day.set(i, day.get(i) + "日");
        for (int i = 0; i < hour.size(); i++)
            hour.set(i, hour.get(i) + "时");
        for (int i = 0; i < minute.size(); i++)
            minute.set(i, minute.get(i) + "分");
    ...
    private void addListener() {
        ...
        hour_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            @Override
            public void onSelect(String text, int index) {
                selectedCalender.set(Calendar.HOUR_OF_DAY, Integer.parseInt(text.substring(0, text.length() - 1)));
                minuteChange();
            }
        });
        ...
        minute_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            @Override
            public void onSelect(String text, int index) {
                selectedCalender.set(Calendar.MINUTE, Integer.parseInt(text.substring(0, text.length() - 1)));
            }
        });
    }
    ...
    public void showTime() {
        if (canAccess) {
           ...
           /*年月日*/
            ...
            hour.clear();
            for (int i = 0; i < 24; i++) {
                hour.add(formatTimeUnit(i) + "时");
            }
            minute.clear();
            for (int i = 0; i < 60; i++) {
                minute.add(formatTimeUnit(i) + "分");
            }
        }
    }
     ...
     * 设置日期控件是否显示年月日
     */
    public void showDateTime(boolean show) {
        if (canAccess) {
            if (show) {
                disScrollUnit();
...
//                year.set(i,year.get(i)+"年");
            ...
            for (int i = 0; i < minute.size(); i++)
                minute.set(i, minute.get(i) + "分");
            if (year_pv.getVisibility() == View.VISIBLE)
                year_pv.setSelected(dateStr[0] + "年");
            if (month_pv.getVisibility() == View.VISIBLE)
                month_pv.setSelected(dateStr[1] + "月");
            if (day_pv.getVisibility() == View.VISIBLE)
                day_pv.setSelected(dateStr[2] + "日");
            if (timeStr == null)
                return;
            if (hour_pv.getVisibility() == View.VISIBLE)
